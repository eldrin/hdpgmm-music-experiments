...
%     \item \textbf{GTZAN~\cite{DBLP:journals/taslp/TzanetakisC02}} provides $1,000$ songs equally partitioned by $10$ genres. We used the fault-filtered split introduced in \cite{DBLP:journals/tmm/KereliukSL15} and measured \emph{f1 score} averaged over genres for measuring the performance.
%     \item \textbf{MTAT} includes $25,863$ music clips from $5,223$ unique songs and annotated with music tags. The tag set contains $188$ music tags in total, where we employed the top $50$ tags and the data split from~\cite{lee_multi-level_2017}
%     \item \textbf{Echonest} 
% \end{itemize}
...
\begin{tabular}{ cccc }
    ...
    MTAT     & $25,863$  & $50$          & ROC-AUC~\cite{DBLP:journals/prl/Fawcett06} \\
    ...
    \hline
\end{tabular}
\caption{Details on dataset used. We use \emph{macro} average strategy for F1 and ROC-AUC measure, and we consider top $500$ recommendationed songs for computing NDCG.}
\label{tab:dataset}
\end{table}
