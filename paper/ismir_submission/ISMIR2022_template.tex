...
\end{aligned}
\end{equation}
where $u_{k}, v_{k}, a_{t}, b_{t}$ denote the variational parameters for the Beta distributions for corpus-level and document-level stick proportions, respectively. $\varphi_{jt}\in\mathbf{R}^{K}, \zeta_{jn}\in\mathbf{R}^{T}$ are the variational parameters for the Multinomial distribution to draw the selector $c$ and $z$.

Notably, we truncate the inifinite Beta distributions by $K$ and $T$, which is a common method for applying VI on the infinite mixture model. With sufficiently large number for the truncation, the model will still not be limited to the truncation, and will only use the number of components that suits for given dataset. Final variational distribution is the Gaussian-Wishart prior distribution which draws the Gaussian parameters for distribution $F$:


\begin{equation}\label{eq:meanfield_normalwishart}
    q(\mu, \Lambda) = \textstyle \prod\nolimits_{k}^{K}\mathcal{N}(\mu_{k}|m_{k}, (\lambda_{k}\Lambda_{k})^{-1})\mathcal{W}(\Lambda_{k}|W_{k}, \nu_{k})
\end{equation}
where we draw the precision $\Lambda_{k}\in\mathbf{R}^{d \times d}$ from Wishart distribution with the variational parameter $W_{k}\in\mathbf{R}^{d \times d}$ and $\nu_{k}\in\mathbf{R}$, and the mean $\mu_{k}\in\mathbf{R}^{d}$ is drawn by the precision weighted by $\lambda_{k}\in\mathbf{R}$ and mean $m_{k}\in\mathbf{R}^{d}$.

We then can obtain the optimal model by maximizing the lowerbound of the marginal log likelihood $\text{log}\,p(X|Z)$ using the standard result of VI~\cite{DBLP:journals/jei/BishopN07, DBLP:journals/ml/JordanGJS99, 10.1214/06-BA104}:


\begin{equation}\label{eq:lowerbound}
\begin{aligned}
    \text{log}\, p(X|Z) \geq \mathbf{E}_{q}[\text{log} p(X, \beta^{\prime}, \pi^{\prime}, c, z, \phi)] + H(q) \\
    = \textstyle \sum_{j} \big\{ \mathbf{E}_{q}[\text{log}(p(X_{j}|c_{j}, z_{j}, \phi))p(z_{j}|\pi_{j}^{\prime})p(\pi^{\prime}_{j}|\alpha_{0})) \\
    + H(q(c_{j})) + H(q(z_{j}) + H(q(\pi^{\prime}_{j})) \big\} \\
    + \mathbf{E}_{q}[\text{log}\,p(\beta^{\prime})p(\phi)] + H(q(\beta^{\prime})) + H(q(\phi))
\end{aligned}
\end{equation}
