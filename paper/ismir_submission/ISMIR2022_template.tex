...
\begin{enumerate}
    \item We explore and suggest insight how ``good'' the music representation learned by Bayesian nonparametric models is.
...
\subsection{Datasets}\label{sec:experimental_setup:datasets}

We used the subset of Million Song Dataset (MSD)~\cite{Bertin-Mahieux2011} as the ``source'' dataset for the representation learning (i.e., inference for HDPGMM). Specifically, we used the preview audio of the ``train'' subset introduced by~\cite{DBLP:conf/ismir/PonsNPSES18,app8010150}\footnote{Length of preview audio differs per clips, where about 70\% of samples are approximately 30 seconds and rest are 1 minute, with a small subset is longer than 1 minute.}. For the evaluation of the representation, we employ three target (downstream) tasks encompassing \emph{music genre classification, music auto tagging, music recommendation}. For each task, we use the GTZAN dataset~\cite{DBLP:journals/taslp/TzanetakisC02} with the fault-filtered split~\cite{DBLP:journals/tmm/KereliukSL15}, MagnaTagaTune (MTAT) dataset~\cite{DBLP:conf/ismir/LawWMBD09} with the split from ~\cite{lee_multi-level_2017}, and finally the Echo Nest taste profile subset as part of MSD~\cite{Bertin-Mahieux2011} filtered by user and item frequency~\cite{DBLP:conf/www/LiangKHJ18} for the genre classification, auto-tagging and recommendation, respectively.
\begin{itemize}[noitemsep, leftmargin=*]
    \setlength\itemsep{0em}
    \item \textbf{MSD} something something something something something something
    \item \textbf{GTZAN} something something something something something something
    \item \textbf{MTAT} something something something something something something
    \item \textbf{Echonest} something something something something something something
\end{itemize}

% \subsubsection{MSD}\label{sec:experimental_setup:datasets:msd}
% \subsubsection{GTZAN}\label{sec:experimental_setup:datasets:gtzan}
% \subsubsection{MTAT}\label{sec:experimental_setup:datasets:mtat}
% \subsubsection{Echonest}\label{sec:experimental_setup:datasets:echonest}

% The details on datasets can be found in Table~\ref{tab:dataset}.
% 
% \begin{table}[h!]
% \centering
% \begin{tabular}{ cccc }
%     \hline
%                             & Id       & \# samples & \# classes / \# users \\
%     \hline
%     \hline
%     Source                  & MSD      & $200,000$ & N/A                    \\
%     \hline
%     \multirow{3}{*}{Target} & GTZAN    & $1,000$   & $10$                   \\
%                             & MTAT     & $25,000$  & $50$                   \\
%                             & Echonest & $40,000$  & $1000,000$             \\
%     \hline
% \end{tabular}
% \caption{Details on dataset used.}
% \label{tab:dataset}
% \end{table}



\subsection{Audio Feature}\label{sec:experimental_setup:feature}

\subsection{Comparisons}\label{sec:experimental_setup:comparisons}

\subsubsection{G1}\label{sec:experimental_setup:comparisons:g1}
\subsubsection{VQ Codebook}\label{sec:experimental_setup:comparisons:vq}
\subsubsection{Kim}\label{sec:experimental_setup:comparisons:kim}
\subsubsection{CMLR}\label{sec:experimental_setup:comparisons:clmr}
\subsubsection{HDPGMM}\label{sec:experimental_setup:comparisons:hdpgmm}

\subsection{Hyper Parameters}\label{sec:experimental_setup:hyperparams}
